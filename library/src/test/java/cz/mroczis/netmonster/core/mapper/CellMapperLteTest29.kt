    ...
    init {
        ...
        "Reflection - RSRP" {
            ...
            mockValidCell().let {
                ...
                it.identity.mapCell(it.info.mapConnection(), it.signal.mapSignal())?.signal?.rsrp shouldBe -102.5
            }

            // Sony E2003
            mockValidCell().let {
                every { it.signal.rsrp } returns 464
                it.identity.mapCell(it.info.mapConnection(), it.signal.mapSignal())?.signal?.rsrp shouldBe -46.4
            }

            // Device name I forgot
            mockValidCell().let {
                every { it.signal.rsrp } returns 25
                it.identity.mapCell(it.info.mapConnection(), it.signal.mapSignal())?.signal?.rsrp shouldBe (-140.0 + 25.0)
            }
        }
        ...
        "Reflection - RSRQ" {
            ...
            mockValidCell().let {
                ...
                it.identity.mapCell(it.info.mapConnection(), it.signal.mapSignal())?.signal?.rsrq shouldBe -13.5
            }
        }
        ...
        "Reflection - SNR" {
            mockValidCell().let {
                ...
                it.identity.mapCell(it.info.mapConnection(), it.signal.mapSignal())?.signal?.snr shouldBe null
            }
